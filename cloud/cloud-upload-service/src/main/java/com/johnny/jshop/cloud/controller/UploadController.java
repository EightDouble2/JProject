...
import com.johnny.jshop.cloud.dto.FileInfo;
...
public class UploadController {
    ...
    public ResponseResult<FileInfo> upload(MultipartFile multipartFile) {
        ...
        try {
            ...
            return new ResponseResult<FileInfo>(ResponseResult.CodeStatus.OK.value(), ResponseResult.CodeStatus.OK.getReasonPhrase(), new FileInfo("http://" + BUCKET_NAME + "." + ENDPOINT + "/" + newName));
        } catch (IOException e) {
            return new ResponseResult<FileInfo>(ResponseResult.CodeStatus.FAIL.value(), ResponseResult.CodeStatus.FAIL.getReasonPhrase());
        } finally {
            client.shutdown();
        }
    }
